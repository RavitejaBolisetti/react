...
const baseAPICall = params => {
    const { method, url, data, onSuccess, onError, onWarning = undefined, onTimeout, onUnauthorized, onUnAuthenticated, postRequest, token, onErrorModal = undefined } = params;
    let axiosConfig = {
        ...
        .catch(error => {
            ...
            if (error.response) {
                ...
                if (error.response.status === 401) {
                    ...
                    onErrorModal && !onWarning && message && validationMessages && validationMessages.length > 0
                        ? onErrorModal(message, validationMessages)
                        : onErrorModal && message && !onWarning ? onError("ERROR", [message])
                            : !onWarning && onError(AXIOS_ERROR_WITH_RESPONSE, error.response.status, message);
