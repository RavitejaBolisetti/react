        ...
        <div className={`${styles.viewContainer} ${styles.hierarchyRightContaner} ${styles.viewProductDetail}`}>
            <Descriptions {...viewProps}>
                ...
                ( <>
                    ...
                    <Descriptions.Item label="Percentage"> {selectedTreeData?.percentage}</Descriptions.Item>
                    ):( calculationType === CALCULTION_TYPE[0]?.key ? (
                    <Descriptions.Item label="Amount">{ selectedTreeData?.rate}</Descriptions.Item>):null)}
                </>)}
