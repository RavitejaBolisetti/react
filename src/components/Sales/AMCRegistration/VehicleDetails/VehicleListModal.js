...
export const VehicleListBase = (props) => {
    ...
    const rowSelection = {
        selectedRowKeys: selectedRows.map((row) => row?.['vehicleIdentificationNumber']),
            ...
            <Row gutter={20}>
                <Col xs={24} sm={24} md={24} lg={24} xl={24} className={styles.buttonsGroupRight}>
                    <Button data-testid="counter-text" htmlType="submit" type="primary" onClick={handleViewDetail} disabled={!formBtnActive}>
                        {translateContent('amcRegistration.label.selectVehicle')}
                    </Button>
                </Col>
            </Row>
        </div>
...
export const VehicleListModal = withModal(VehicleListBase, { width: 800 });
