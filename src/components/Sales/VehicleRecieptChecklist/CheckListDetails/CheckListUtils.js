...
import { convertDateTime, formatDateToCalenderDate } from 'utils/formatDateTime';
    ...
    switch (type) {
        case FORMTYPE_CONSTANTS?.DATE?.id: {
            if (data?.answerFromDate && data?.answerToDate) {
                checkResult = checkResult.concat(data?.answerFromDate ? convertDateTime(data?.answerFromDate) : 'NA');
                checkResult = checkResult.concat('-');
                checkResult = checkResult.concat(data?.answerToDate ? convertDateTime(data?.answerToDate) : 'NA');
                return checkResult;
            } else {
                return 'NA';
            }
