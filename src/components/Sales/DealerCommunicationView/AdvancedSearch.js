...
export const AdvancedSearchFrom = (props) => {
    const { isDataCountryLoaded,financialYearList,  countryData, defaultCountry, handleFilterChange, filteredStateData, filteredDistrictData } = props;
        ...
        <Form autoComplete="off" layout="vertical" form={advanceFilterForm} onFinish={onFinish}>
            <Row gutter={16}>
                <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Form.Item initialValue={defaultCountry} label={'Financial Year' || translateContent('city.label.countryCode')} name="financial Year" rules={[validateRequiredSelectField('Financial Year' || translateContent('city.validation.country'))]}>
                ...
                <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Form.Item label={'Financial Month' || translateContent('city.label.state')} initialValue={filterString?.stateCode} name="stateCode">
                        <Select placeholder={'Financial Month' || translateContent('city.placeholder.select')} {...selectProps} onChange={handleFilterChange('stateCode')}>
                ...
                <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                    <Form.Item label={'Scheme Type' || translateContent('city.label.state')} initialValue={filterString?.stateCode} name="stateCode">
                        <Select placeholder={'Scheme Type' || translateContent('city.placeholder.select')} {...selectProps} onChange={handleFilterChange('stateCode')}>
                            {filteredStateData?.map((item) => (
                                <Option key={item?.key} value={item?.key}>
                                    {item?.value}
                                </Option>
