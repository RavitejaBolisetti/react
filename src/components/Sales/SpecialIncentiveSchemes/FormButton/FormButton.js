...
export const FormButton = ({ record, onCloseAction, handleShowBudgetModel=()=>{}, buttonData, setButtonData, saveButtonName = translateContent('global.buttons.continue'), handleButtonClick, isLoadingOnSave, isLastSection, buttonType = false }) => {
    return (
        <div className={styles.formFooter}>
            <Row gutter={20}>
                ...
                {buttonData?.viewBudget && (
                        <Button onClick={() => handleShowBudgetModel({ record })} type="primary">
                            {'View Budget'}
                        </Button>
                    )}
                    {buttonData?.editBtn && (
