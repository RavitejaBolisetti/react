...
export default function RegistrationFilter(props) {
    const { userType, extraParams, removeFilter, typeData, handleStatusTypeChange, currentStatusButton, statusBtnList, invoiceStatusList, searchForm, filterString, setFilterString, handleResetFilter, advanceFilter = false, handleInvoiceTypeChange, setAdvanceSearchVisible, handleButtonClick, amcStatus, showAddButton } = props;
            ...
            <Row gutter={20}>
                    ...
                    <Form autoComplete="off" colon={false} className={styles.masterListSearchForm}>
                        <Row gutter={20}>
                            <Col xs={24} sm={24} md={20} lg={20} xl={20} className={styles.verticallyCentered}>
                                <QueryButtons currentItem={currentStatusButton} items={statusBtnList} onClick={handleStatusTypeChange} />
                ...
                {showAddButton && (
                    <Col xs={24} sm={4} md={4} lg={4} xl={4} className={styles.buttonsGroupRight}>
                        <Button icon={<PlusOutlined />} className={styles.actionbtn} type="primary" onClick={() => handleButtonClick({ buttonAction: FROM_ACTION_TYPE?.ADD, type: 'addSpclClaimType' })}>
