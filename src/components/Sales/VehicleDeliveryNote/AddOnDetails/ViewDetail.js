...
const ViewDetailMain = (props) => {
    const { formData, isLoading, typeData, relationshipManagerData, schemeAmcData } = props;
                ...
                <Col xs={24} sm={24} md={24} lg={24} xl={24}>
                    <Collapse expandIcon={expandIcon} activeKey={activeKey} onChange={() => onChange(1)} expandIconPosition="end">
                        <Panel header={translateContent('vehicleDeliveryNote.addOnDetails.heading.shieldInfo')} key="1">
                            ...
                            <Descriptions {...viewProps}>
                                ...
                                <Descriptions.Item label={translateContent('vehicleDeliveryNote.addOnDetails.label.saleType')}>{checkAndSetDefaultValue(getCodeValue(typeData?.SALE_TYP, formData?.sheildRequest?.saleType, isLoading))}</Descriptions.Item>
                    ...
                    <Collapse expandIcon={expandIcon} activeKey={activeKey} onChange={() => onChange(2)} expandIconPosition="end">
                        <Panel header={translateContent('vehicleDeliveryNote.addOnDetails.heading.rsa')} key="2">
                            ...
                            <Descriptions {...viewProps}>
                                ...
                                <Descriptions.Item label={translateContent('vehicleDeliveryNote.addOnDetails.label.saleType')}>{checkAndSetDefaultValue(getCodeValue(typeData?.SALE_TYP, formData?.rsaRequest?.saleType, isLoading))}</Descriptions.Item>
                    ...
                    <Collapse expandIcon={expandIcon} activeKey={activeKey} onChange={() => onChange(3)} expandIconPosition="end">
                        <Panel header={translateContent('vehicleDeliveryNote.addOnDetails.heading.amc')} key="3">
                            <Divider />
                            <Descriptions {...viewProps}>
                                <Descriptions.Item label={translateContent('vehicleDeliveryNote.addOnDetails.label.schemeType')}>{checkAndSetDefaultValue(getCodeValue(typeData?.DLVR_AMC_SCH_TYP, formData?.amcRequest?.schemeType), isLoading)}</Descriptions.Item>
                                ...
                                <Descriptions.Item label={translateContent('vehicleDeliveryNote.addOnDetails.label.schemeDescription')}>{checkAndSetDefaultValue(getCodeValue(schemeAmcData, formData?.amcRequest?.schemeDescription, 'schemeDescription', true, 'schemeCode'), isLoading)}</Descriptions.Item>
                                <Descriptions.Item label={translateContent('vehicleDeliveryNote.addOnDetails.label.saleType')}>{checkAndSetDefaultValue(getCodeValue(typeData?.SALE_TYP, formData?.amcRequest?.saleType), isLoading)}</Descriptions.Item>
                                <Descriptions.Item label={translateContent('vehicleDeliveryNote.addOnDetails.label.valiFromDate')}>{checkAndSetDefaultValue(formData?.amcRequest?.validFrom, isLoading, DATA_TYPE?.DATE?.key)}</Descriptions.Item>
                                <Descriptions.Item label={translateContent('vehicleDeliveryNote.addOnDetails.label.validToDate')}>{checkAndSetDefaultValue(formData?.amcRequest?.validTo, isLoading, DATA_TYPE?.DATE?.key)}</Descriptions.Item>
