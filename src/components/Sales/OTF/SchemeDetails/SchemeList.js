            ...
            {!isViewModeVisible
                ? contactData?.length > 0 &&
                  contactData?.map((data, i) => {
                      return (
                          <Collapse onChange={() => handleCollapse(i)} expandIconPosition="end">
                              ...
                              <Panel
                                  ...
                                  key={i}
                              >
                                  <ViewSchemeDetails styles={styles} formData={data} />
                              </Panel>
                          </Collapse>
                      );
                  })
                : formData?.length > 0 &&
                  formData?.map((data) => {
                      return <ViewSchemeDetails styles={styles} formData={data} />;
                  })}
        </div>
    );
};
