...
import { DistrictGeo } from '../Geo/District/DistrictGeo';
...
jest.mock('react-redux', () => ({
    connect: () => (DistrictGeo) => DistrictGeo,
}));
...
describe('PIN Master Test', () => {
    test('Is the search Field Present or not', () => {
        render(<DistrictGeo fetchList={fetchList} saveData={saveData} />);
        searchFieldTest()
    });

    test('Is the Refresh Button Present or not', () => {
        render(<DistrictGeo qualificationData={qualificationMasterData1} fetchList={fetchList} saveData={saveData} />);
    ...
    //     render(<DistrictGeo qualificationData={qualificationMasterData1} listShowLoading={listShowLoading} fetchList={fetchList} saveData={saveData} />);
    //     searchFunctionality('ZHJ')
    // });

    test('is drawer closing on click of cancel button', async () => {
        render(<DistrictGeo qualificationData={qualificationMasterData} fetchList={fetchList} saveData={saveData} />);
