...
export const DrawerFormButton = ({ formData, id = null, onCloseAction, buttonData, setButtonData, saveButtonName = 'Save', handleButtonClick, isLoadingOnSave }) => {
    return (
        <Row gutter={20} className={styles.formFooter}>
            ...
            <Col xs={24} sm={12} md={12} lg={12} xl={12} className={styles.footerBtnRight}>
                {buttonData?.saveBtn && (
                    <Button form={id} loading={isLoadingOnSave} disabled={!buttonData?.formBtnActive} onClick={(e) => setButtonData({ ...buttonData, saveAndNewBtnClicked: false })} htmlType="submit" type="primary">
                        {saveButtonName}
                    </Button>
                )}
