...
const ProfileDetailCard = (props) => {
    ...
    // const userAvatar = formData?.userName();
    ...
    return (
        <Collapse bordered={true} defaultActiveKey={[1]} accordion={true} expandIcon={expandIcon} collapsible="icon">
                ...
                header={
                    <>
                        <Space>
                            ...
                            <div>
                                <Title level={5}>{selectedRecord?.customerName}</Title>
                                <Text>{selectedRecord?.customerId}</Text>
                            </div>
                        </Space>
                    </>
                }
            ...
            >
               {userType === USER_TYPE_USER?.DEALER?.id ? (
                <>
                    ...
                    <Divider />
                    <p>
                        Token No: <span>{selectedRecord?.employeeCode}</span>
                    </p>
                </>
            ) : (
                <p>
            ...
            <Divider />
            <p>
                Mobile Number: <span>{selectedRecord?.mobileNumber}</span>
            </p>
            <Divider />
            <p>
                Email ID: <span>{selectedRecord?.email}</span>
            </p>
            </Panel>
        </Collapse>
    );
