                    ...
                    <Col xs={18} sm={18} md={18} lg={18} xl={18} xxl={18}>
                        <Row>
                            <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
                                {'Brand Code : ' || translateContent('applicationMaster.text.code')}
                                <Text strong>{formData?.brandCode}</Text>
                            </Col>
                            <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
                                {'Brand Description : ' || translateContent('applicationMaster.text.code')}
                                <Text strong> {formData?.brandDescription}</Text>
                            </Col>
                            <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
                                {'Status : ' || translateContent('applicationMaster.text.code')}
                                {/* <Text strong> {formData?.isActive}</Text> */}
                                <span className={formData?.isActive ? styles.activeText : styles?.inactiveText}>{formData?.isActive ? translateContent('global.label.active') : translateContent('global.label.inActive')}</span>
                            </Col>
                        </Row>
                    </Col>
