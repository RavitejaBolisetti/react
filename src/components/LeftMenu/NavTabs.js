            ...
            <div className={`flex flex-col mt-3 1x1:gap-[8px] gap-[6px] mb-6 z-10 relative`}>
                ...
                <button type="button" className={`flex flex-col ${isOpen ? 'justify-center' : 'justify-between'} items-center w-full mb-[4px] relative`}>
                    <div onClick={() => setSubmenu(!submenu)} className={`flex items-center ${isOpen ? 'justify-center' : 'justify-between'} w-full text-[#FF3E5B]`}>
                        ...
                        {!isOpen && <BsChevronDown size={18} className={`text-[#FF3E5B] ${!submenu && 'rotate-180 transition-all'} transition-all`} />}
                    </div>
                    {/* submenu */}
                        ...
                        {!submenu && !isOpen && (
                            <>
                                <ul className="w-full mt-[0.4rem] flex flex-col gap-1">
                                    {SubMenuList.map((ele) => {
                                        return (
                                            <li onClick={() => setActiveTabs(ele.id)} key={ele.id} className={`w-auto whitespace-nowrap ${activeTabs === ele.id ? `text-[#FF3E5B] ${theme === 'dark' ? 'bg-[#0B0B0C]' : 'bg-white'}` : `${theme === 'dark' ? 'text-white hover:bg-[#0B0B0C]' : 'text-[#342C2C] hover:bg-white'}`} text-left pl-[30px] rounded py-[5px] lg:text-[14px] text-[12px]`}>
                                                {ele.list}
                                            </li>
                                        );
                                    })}
