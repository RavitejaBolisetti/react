...
import { B_DARK_THEME, B_LIGHT_THEME, ROBIN_DARK_THEME, ROBIN_LIGHT_THEME } from '../../assets';
        ...
        <>
            <nav className={` overflow-hidden ${isOpen ? 'slg:w-[60px] -w-[120%]' : 'slg:w-[240px] w-full'} h-full ${theme === 'dark' ? 'bg-[#242424]' : 'bg-[#F4F4F4]'} slg:transition-none transition-all slg:duration-none duration-1000 px-[14px] pt-[14px] flex flex-col justify-between select-none shadow-[0_0_3px_rgba(0,0,0,0.2)]`}>
                <div className="w-full">
                        ...
                        <button type="button" className="flex items-center justify-between w-full mb-[10px]" onClick={() => setIsOpen(!isOpen)}>
                            {isOpen ? (
                                <div className="w-[15px] h-[36px] flex items-start transition-all duration-300 relative left-[4px]">
                                    <img src={`${theme === 'dark' ? B_DARK_THEME : B_LIGHT_THEME}`} className="h-full mr-auto" alt="robin-logo" />
                                </div>
                            ) : (
                                <div className="h-[36px] flex items-center py-[5px]">
                                    <img src={`${theme === 'dark' ? ROBIN_DARK_THEME : ROBIN_LIGHT_THEME}`} className="h-full mr-auto" alt="robin-logo" />
                                </div>
                            )}
                    ...
                    {/* search boxs */}
                    <div className={`w-full ${isOpen ? 'justify-center' : ''} h-[34px] flex items-end`}>{isOpen ? <AiOutlineSearch size={25} /> : <input className={`${theme === 'dark' ? 'bg-[#0B0B0C] text-white' : 'bg-white text-black'} w-full h-full text-[14px] placeholder:text-[#8E8585] p-[6px_12px] rounded ${isOpen && 'scale-0'}`} placeholder="Search menu.." style={{ border: `1px solid ${theme === 'dark' ? '#342C2C' : '#DEDEDE'}` }} />}</div>
