...
const props = {
    ...
    customerDetailsData: [{id: 252, value: "test2"}, {id: 25, value: "test"}]
}
...
describe('customer details master', () => {
    const mockStore = createMockStore({
        ...
        data: {
            CustomerMaster: {
                CustomerDetails: { isLoaded: false, isLoading: false, data: [{ id: 123, value: 'testDMS' }, { id: 32, value: 'testDMS' }] },
                Corporate: { isFilteredListLoaded: false, isLoading: false, filteredListData: dcorporateLovData },
                CustomerParentCompany: { isLoaded: false, isCustomerParentCompanyLoading: false, data: [{ id: 123, value: 'testDMS' }, { id: 1313, value: 'testDMS' }] },
            },
            ConfigurableParameterEditing: { filteredListData: [{ id: 123, value: 'testDMS' }, { id: 321, value: 'testDMS' }] },
        },
    });
    ...
    it('should render customer details add edit form', async () => {
        const { getByRole } = customRender(
            <Provider store={mockStore}>
                <FormWrapper  {...props}
                    ...
                    handleCorporateChange={jest.fn()}
                    validateParentCode={jest.fn()}
                    handleButtonClick={jest.fn()}
                    setCorporateType={jest.fn()}
                    onHandleSelect={jest.fn()}
                />
            </Provider>
        )
        ...
        const parentCompanyCode = getByRole("textbox", { name: 'Parent Company Code', exact: false });
        fireEvent.change(parentCompanyCode, { target: { value: '87387' } });

        const parentCompanyName = getByRole("textbox", { name: 'Parent Company Name', exact: false });
        ...
        const corporateType = getByRole("combobox", { name: 'Corporate Type', exact: false });
        fireEvent.change(corporateType, { target: { value: 'test' } });

        const corporateName = getByRole("combobox", { name: 'Corporate Name', exact: false });
