...
const finalFormdata ={ documentType: [ {termAndConRequired:true, digitalSignatureRequired:true, documentTypeDescription:'Test', documentTypeCode:'123'},
...
describe('Card Document Type Component', () => {
    ...
    it('edit icon and save button should work', async () => {
        ...
        render(<FormWrapper onUpdate={jest.fn()} onClick={jest.fn()} forceUpdate={jest.fn()} setfinalFormdata={setfinalFormdata} setCanFormSave={jest.fn()} setIsBtnDisabled={jest.fn()}  />)
        const buttons = screen.getAllByRole('button', { name: '', exact: false });
        await act(async () => {
