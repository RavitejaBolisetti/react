...
describe('Dealer company components', () => {
    it('Should render add edit form components', () => {
        ...
        const pincodeData = [
            { key: 1, id: 1, value: 'test', pinCode: '841701' },
            { key: 2, id: 2, pinCode: '4545', value: 'test' },
        ];
        const fetchPincodeDetail = jest.fn();
        customRender(<FormWrapper isVisible={true} showGlobalNotification={jest.fn()} formData={formData} formActionType={formActionType} setButtonData={jest.fn()} handleFormValueChange={jest.fn()} fetchPincodeDetail={fetchPincodeDetail} pattern={pattern} pincodeData={pincodeData} checked={true} isDataLoaded={false} userId="12345" />);

        const searchInput = screen.getByPlaceholderText('Search');
        fireEvent.change(searchInput, { target: { value: '123454' } });
        expect(searchInput).toBeInTheDocument();

        const pinCode = screen.getByRole('combobox', { name: 'Pin Code' });
        fireEvent.change(pinCode, { target: { value: '841703' } });
    ...
    it('Should render cancel button', () => {
        ...
        const buttonData = { saveAndNewBtn: true, cancelBtn: true, closeBtn: true };
        const formData = { companyCode: 'test', parentCode: 'test', status: true };
        customRender(<FormWrapper isVisible={true} formData={formData} buttonData={buttonData} handleButtonClick={jest.fn()} onCloseAction={jest.fn()} setButtonData={jest.fn()} handleFormValueChange={jest.fn()} formActionType={formActionType} />);
