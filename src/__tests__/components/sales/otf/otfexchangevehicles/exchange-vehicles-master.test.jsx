...
describe("onSearch",()=>{
    ...
    it("onSearch when no value", ()=>{
        ...
        customRender(<ExchangeVehiclesMaster {...props} 
            typeData={('REL_TYPE', 'MONTH')} 
            setButtonData={jest.fn()}
            buttonData={defaultBtnVisiblity}
            onSearch={jest.fn(value)}
        />)
    ...
    it("Make combobox", ()=>{
        ...
        customRender(
            <FormWrapper {...props} 
                ...
                setButtonData={jest.fn()}
                buttonData={defaultBtnVisiblity}
                setfilteredModelData={jest.fn()}
                setfilteredVariantData={jest.fn()}
                handleFilterChange={jest.fn(name,value,selectobj)}
                makeData={makeData}
            />
        )
        ...
        fireEvent.change(make, {target:{value:'Maruti'}})
        // act(() => {
        //     fireEvent.change(make, { target: { value: 'Maruti' } });
        //     const kai = screen.getByText('Maruti');
        //     fireEvent.click(kai);
        // });
    })
